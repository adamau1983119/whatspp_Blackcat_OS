...
async function handleMessage(text, principalId, rawCtx) {
  ...
  try {
    ...
    if (session.osState === 'GAME_HUB') {
      ...
      if (isBotOwnReply(trimmed, session.locale)) return ok(null);
    ...
    if (session.osState === 'APP_ACTIVE' && session.currentApp === 'MAIL') {
      if (isBotOwnReply(trimmed, session.locale)) return ok(null);
      const reply = await dispatchPluginAsync(
        { type: 'SYS_MAIL', locale: session.locale, payload: ctx.text },
        session,
        ctx
      );
      return ok(reply);
