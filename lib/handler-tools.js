...
const { loadEmailRoutes, buildMailDevicePrompt, enterMailMode } = require('./mail-parse');
const { dispatchPluginAsync, PLUGIN_TYPES } = require('./plugin-dispatch');
const { enterMenu } = require('./session');
const { isBotOwnReply } = require('./bot-reply-guard');
...
async function handleToolsHubDigit(session, principalId, text, ctx) {
  ...
  if (toolType === 'SYS_MAIL') {
    enterMailMode(session);
    return buildMailDevicePrompt(session.locale, loadEmailRoutes());
  }
...
async function routeToolsHub(session, principalId, trimmed, cmd, ctx) {
  ...
  if (isBotOwnReply(trimmed, session.locale)) return null;
