...
const { parsePluginPrefix: matchPluginPrefix } = require('./parse-plugin-prefix');
...
function parsePluginPrefix(text) {
  loadCommands();
  return matchPluginPrefix(text, commandsCache.plugins);
}
...
function parseCommand(text, sessionLocale, ctxSource) {
  ...
  const plugin = parsePluginPrefix(trimmed);
