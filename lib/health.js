/**
 * health.js — Railway 健康檢查 + 網頁 QR + 郵件中轉頁
 */

const http = require('http');
const { URL } = require('url');
const { buildMailtoUrl, publicBaseUrl, buildMailOpenHtml } = require('./mailto');
...
function handleMailRedirect(req, res) {
  const base = publicBaseUrl();
  const u = new URL(req.url || '/mail', base);
  const subject = u.searchParams.get('subject') || '';
  const body = u.searchParams.get('body') || '';
  const mailto = buildMailtoUrl('', subject, body);
  res.writeHead(200, { 'Content-Type': 'text/html; charset=utf-8' });
  res.end(buildMailOpenHtml(mailto));
}

/** 啟動 HTTP 伺服器（健康檢查 + QR 頁 + 郵件中轉） */
function startHealthServer() {
  const port = Number(process.env.PORT) || 3000;
  const server = http.createServer((req, res) => {
    const path = String(req.url || '').split('?')[0];
    if (path === '/mail') {
      handleMailRedirect(req, res);
      return;
    }
    res.writeHead(200, { 'Content-Type': 'text/html; charset=utf-8' });
    res.end(buildHtml());
