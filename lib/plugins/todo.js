...
function execute(cmd, session, ctx) {
  ...
  if (cmd.type === 'SYS_TODO_LIST') {
    ...
    return withQuickFooter(loc, listTodos(session, loc));
  }
  ...
  releaseToIdle(session.principalId);
  if (calendarUrl) {
    return withQuickFooter(loc, t(loc, 'todoSavedWithCalendar', { text, calendarUrl }));
  }
  return withQuickFooter(loc, t(loc, 'todoSaved', { text }));
}
