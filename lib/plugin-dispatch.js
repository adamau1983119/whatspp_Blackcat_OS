...
async function dispatchPluginAsync(cmd, session, ctx) {
  ...
  try {
    ...
    if (e.message === 'PLUGIN_TIMEOUT') return t(session.locale, 'pluginTimeout');
    return t(session.locale, 'pluginError');
  }
}
...
async function executeWithTimeout(principalId, locale, runPlugin) {
  try {
    ...
    return { reply: t(locale, e.message === 'PLUGIN_TIMEOUT' ? 'pluginTimeout' : 'pluginError') };
  }
}
